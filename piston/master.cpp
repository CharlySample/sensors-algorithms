...
void musculos(double t, double Fi1, double Fi2)
{
	xt = resorte_amortiguador(t,0.8465,0.7,1,0.15,0.2);
	xt1 = resorte_amortiguador(t,1.5465,0.45,1,0.15,0.2);

        baseax = -237*cos(gradtorad(Fi1));
	baseay = 352*sin(gradtorad(Fi1));

	pistonax = -237*cos(gradtorad(Fi1))+xt;
	pistonay = 352*sin(gradtorad(Fi1))-xt;
...
double resorte_amortiguador(double t,double kt, double ct,double m, double a1, double a2)
{
...
void resorte_amortiguador1(double t,double kt, double ct,double m, double a1, double a2)
{
        raiz = pow(ct,2)-4*m*kt;
        ...
        {
                cout << "sobreamortiguado" << endl << endl;
                xt1 = a1*exp(positivo*t)+a2*exp(negativo*t);
        }

        else if(raiz == 0)
        {
          cout << "criticamente amortiguado" << endl << endl;
          xt1 = (a1+a2*t)*exp(-ct*t/2*m);
        }

        else if(raiz < 0)
        {
          cout << "subamortiguado" << endl << endl;
          xt1 = a1*exp(-ct*t/2*m)*cos(sqrt((kt/m)+pow((ct/2*m),2))*t+0.1);
        }
...
BOOST_PYTHON_MODULE(MoveModule)
{
        ...
        .def("resorteamortiguador1", &Movement::resorte_amortiguador1)
        .def_readwrite("ResultXT1", &Movement::xt1)
        ;
}
